...
from SuperNet.networks import MobileNetV3
...
class OFAMobileNetV3(MobileNetV3):
    def __init__(
        ...
        base_stage_width = [16, 16, 24, 40, 80, 112, 160, 512, 960]
    ...
    def forward(self, x):
        ...
        cnn_feature = self.final_expand_layer(x)
        #x = x.mean(3, keepdim=True).mean(2, keepdim=True)  # global average pooling
        x = cnn_feature.mean(3, keepdim=True).mean(2, keepdim=True)
        x = self.feature_mix_layer(x)
        x = x.view(cnn_feature.size(0), -1)
        x = self.classifier(x)
...
if __name__ == "__main__":
    ...
    summary(max_subnet, (3, 256, 256))
    #config = net.sample_active_subnet()
    min_config = {"ks": [3 for i in range(20)], "e":[3 for i in range(20)], "d": [2 for i in range(5)]}
    net.set_active_subnet(ks=min_config["ks"], e=min_config["e"], d=min_config["d"])
    min_subnet = net.get_active_subnet()
    #print(config)
    for i in range(100):
        start_time = time.time()
        output = min_subnet(tensor)
        end_time = time.time()
        #print(end_time - start_time)
        min_time.append(end_time - start_time)
    print("min time:mean {}, std {}".format(np.mean(min_time), np.std(min_time)))
    summary(min_subnet, (3, 256, 256))
    #net.set_active_subnet(ks=[3, 5, 7], e=[3, 6, 6], d=[2, 3, 4])
    # print(net.module_str)
    # print(net.config)
    # print(net.sample_active_subnet())
    # print(net.get_active_subnet())
    # print(net.get_active_net_config())
