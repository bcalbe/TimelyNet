...
class TCP(nn.Module):
	...
	def __init__(self, config,backbone_name = "Superres"):
		...
		if backbone_name == "Res34":
			...
			cnn_dim = 512
		elif backbone_name == "Res50":
			self.perception = resnet50(pretrained=True)
			cnn_dim = 2048
		elif backbone_name == "Res101":
			self.perception = resnet101(pretrained=True)
			cnn_dim = 2048
		elif backbone_name == "Res152":
			self.perception = resnet152(pretrained=True)
			cnn_dim = 2048
			...
			self.perception = OFAResNets(
												...
												#large
												#depth_list=[0,1,2],
												#expand_ratio_list=[0.2,0.4,0.6,0.8],
												#width_mult_list=[1.0],
												# depth_list=[4,6,8],
												# expand_ratio_list=[0.6,0.8,1.0],
												# width_mult_list=[1.0],
											)
			cnn_dim = 2048
			self.perception.set_max_net()
			#self.perception.set_min_net()
	...
	def forward(self, img, state, target_point):
		...
		for i in range(6):
			...
			cnn_feature_list.append(cnn_feature)
		feature_emb = torch.mean(torch.stack(feature_emb_list), dim=0)
		cnn_feature = torch.mean(torch.stack(cnn_feature_list), dim=0)
		self.feature = feature_emb
		tp2 = time()
		#self.encoder_time.append(tp2-tp1)
		...
		#initial hidden states of traj which are the concatenation of average pooled image feature and measurement feature
		z = j_traj
		output_wp = list()
